...
magiczoomswatch.prototype = {
    ...
    mouseOverAttributeItem:function(attribute_id,option_id)
    {
    ...
    appendSelectNode:function(attribute_id)
    {
        ...
        $j("#attribute"+attribute_id+" option").each(function()
        {
            if($j(this).val()!='')
            {
                li = proxy.createLiNodeByOption(attribute_id,$j(this).val(),$j(this).text());
                $j('#custom-'+attribute_id).append(li);
            }
        });
    },
    createLiNodeByOption:function(attribute_id, id,value)
    {
