...
class Flagbit_Faq_Block_Frontend_List extends Mage_Core_Block_Template
{
	...
	public function getFaqCollection($pageSize = null)
	{
		if (!$this->_faqCollection || (intval($pageSize) > 0
			...
			$this->_faqCollection = Mage :: getModel('flagbit_faq/faq')
				...
				->addIsActiveFilter()
				;
			 $this->_faqCollection->getSelect()->order('sort DESC');
			//echo $this->_faqCollection->getSelect();
	...
	public function getCategoryCollection()
	{
	    ...
	    if (is_null($categories)) {
    	    $categories =  Mage::getResourceSingleton('flagbit_faq/category_collection')
    	       ->addStoreFilter(Mage::app()->getStore())
    	       ->addIsActiveFilter();
    	    $categories->getSelect()->order('sort DESC');
	...
	{
	     $collection = $category->getItemCollection()->addIsActiveFilter()->addStoreFilter(Mage::app()->getStore());
	     $collection->getSelect()->order('sort DESC');
	     return $collection;
	}
